...
static int setup_stream(struct alsa_device *dev, snd_pcm_t *pcm, const char *stream_name)
{
	...
	ret = snd_pcm_hw_params_set_period_size_near(pcm, hw_params, &rsize, NULL);
	if (ret < 0) {
